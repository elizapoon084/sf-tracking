...
def _parse_order(raw: str) -> dict:
    ...
    for m in ITEM_RE.finditer(raw):
        sku, qty = m.group(1), int(m.group(2).lower().replace('l', '1'))
        last_end = m.end()
    ...
    if nm:
        ...
        if nm2:
            ...
            if nm3:
                ...
                # 最後手段：全文搵電話前的中文名（含「一」等字）
                nm4 = re.search(
                    r"([^\d\s，,、—\-]{2,10})\s*(?=1[3-9]\d{9}|[2-9]\d{7})", raw)
                if not nm4:
...
# 刪除 session 記錄，防止 Chrome 下次開啟彈「未正確離開」還原通知
for lf in ["Last Session", "Last Tabs", "Current Session", "Current Tabs"]:
    for sub in ["", "Default"]:
        try:
            os.remove(os.path.join(CHROME_PROFILE, sub, lf))
        except Exception:
            pass
...
def download_pos_word(ctx, pos_order_no, save_dir, customer_name):
    ...
    try:
        ...
        try:
            ...
            # 重排頁序：報關(原第3頁)→第1，小票(原第1頁)→第2，刪除原第2頁
            try:
                import fitz as _fitz
                _doc = _fitz.open(combined_path)
                if len(_doc) >= 3:
                    _new = _fitz.open()
                    _new.insert_pdf(_doc, from_page=2, to_page=2)  # 報關→第1
                    _new.insert_pdf(_doc, from_page=0, to_page=0)  # 小票→第2
                    _doc.close()
                    _new.save(combined_path, garbage=4, deflate=True)
                    _new.close()
                    print(f"  ✅ 頁序已調整：報關→第1，小票→第2（共2頁）")
                else:
                    _doc.close()
            except Exception as _pe:
                print(f"  ⚠️  頁序調整失敗：{_pe}")
        ...
        for order_idx, order in enumerate(batch_orders):
            ...
            try:
                ...
                for item in DEMO_POS_ITEMS:
                    try:
                        # 每次加貨前先 Escape，防止 Ctrl+F 搜尋欄被誤觸導致 click 落空
                ...
                pos_page.bring_to_front()
                time.sleep(0.3)

                # 逐個 method 試，確認按鈕消失才算成功
                _methods = [
                    ("force click",     lambda: _confirm_btn.click(force=True)),
                    ("dispatch_event",  lambda: _confirm_btn.dispatch_event('click')),
                    ("tap",             lambda: _confirm_btn.tap(force=True)),
                    ("mouse.click",     lambda: pos_page.mouse.click(
                        *pos_page.evaluate("""() => {
                            const b = [...document.querySelectorAll('button')]
                                .find(b => b.offsetParent && b.textContent.includes('確認，出小票'));
                            if (!b) return [0,0];
                ...
                for _mname, _mfn in _methods:
                    ...
                    if not pos_page.locator("button:has-text('確認，出小票')").is_visible(timeout=500):
                        print(f"  ✅ 確認按鈕成功（{_mname}）")
                        break
                    print(f"  ⚠️  {_mname} 未生效，試下一個...")
                time.sleep(2)
                ...
                for idx, item in enumerate(DEMO_SF_ITEMS):
                    ...
                    # v61: 0100433/0100434 香薰塊跳過材質，其他照舊填
                    _SKIP_MATERIAL_SKUS = {"0100433", "0100434"}
                    if item.get("sku") in _SKIP_MATERIAL_SKUS:
                        print("  ⏭️  材質欄位跳過（香薰塊產品）")
                    else:
                        mat_filled = False
                ...
                # 下單後 SF 可能開新 tab 並關閉原頁，需切換過去
                try:
                    _ = sf_page.url  # 測試頁面是否仍存活
                except Exception:
                    print("  ⚠️  下單後原頁已關閉，嘗試切換到最新頁面")
                    _alive = [p for p in ctx.pages if not p.is_closed()]
                    if _alive:
                        sf_page = _alive[-1]
                        print(f"  切換到: {sf_page.url}")
                        time.sleep(2)

                try:
                    shot(sf_page, f"{order_idx+1:02d}_04_sf_submitted")
                except Exception as _se:
                    print(f"  ⚠️  截圖失敗（不影響流程）: {_se}")
