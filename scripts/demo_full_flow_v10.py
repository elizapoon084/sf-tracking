...
def reprint_one_waybill(ctx, entry: dict) -> bool:
    ...
    try:
        ...
        # 5a. 選「二維碼運單」tab（v10 新增）
        print("  5a. 選「二維碼運單」tab")
        _qr_clicked = page.evaluate("""() => {
            for (const el of document.querySelectorAll('[class*="tabItem"],[class*="printTab"] span')) {
                const t = (el.textContent || '').trim();
                if (t.includes('二維碼') || t.includes('二维码')) {
        ...
        if _qr_clicked:
            ...
            print("     ⚠️  找不到「二維碼運單」tab，繼續嘗試列印")
        time.sleep(2)

        # 5b. 點「列印運單」→ 新 tab → CDP 儲存 PDF
        print("  5b. 點「列印運單」並儲存 PDF")
        try:
            with page.context.expect_page(timeout=20000) as new_pg_info:
                page.evaluate("""() => {
                    const labels = ['列印運單','打印運單','列印此頁面','列印頁面'];
