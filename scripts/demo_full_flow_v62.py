...
def upload_batch_and_get_waybills(page, excel_path: str, orders_with_ids: list) -> dict:
    ...
    def _js_click_confirm(pg):
        """用 JS 點擊頁面上任何顯示中的「確認」按鈕，返回是否成功。"""
        return pg.evaluate("""
            () => {
                const btns = Array.from(document.querySelectorAll('button'));
                const btn = btns.find(b => b.offsetParent !== null && b.textContent.trim() === '確認');
    ...
    # 先去 MonthCard 確認 session
    page.goto(CAMP_URL, wait_until="domcontentloaded")
    time.sleep(3)
    ...
    if _is_logged_out(page):
        ...
        time.sleep(4)
        if _is_logged_out(page):
            raise RuntimeError("camp.sf-express.com 仍未登入，請重試")

    ok = _goto_batch_page(page)
    if not ok:
        page.screenshot(path=os.path.join(ORDERS_DIR, f"_batch_nav_fail_{today}.png"))
        raise RuntimeError(f"無法載入批量上傳頁 file input，URL={page.url}，截圖已存 _batch_nav_fail_{today}.png")

    # 上傳 Excel — input[type=file] 在 el-upload 裡是隱藏的，直接 set_input_files
    uploaded = False
    for sel in ["input[type=file]", ".el-upload input[type=file]", "input[accept]"]:
        try:
            el = page.locator(sel).first
            el.set_input_files(excel_path)  # Playwright 可對隱藏 input 直接設檔案
            uploaded = True
            print(f"  已上傳（{sel}）")
            break
        except Exception:
            continue
    if not uploaded:
        ss = os.path.join(ORDERS_DIR, f"_upload_debug_{today}.png")
        page.screenshot(path=ss)
        url = page.url
        body = page.inner_text("body")[:500]
        print(f"  當前 URL: {url}")
        print(f"  頁面文字(前500): {body}")
        print(f"  截圖: {ss}")
        raise RuntimeError(f"找不到上傳 input，URL={url}")
    time.sleep(6)
    _dismiss(page)
...
with sync_playwright() as pw:
    ...
    try:
        ...
        print(f"  ⚠️  Excel 歷史存檔跳過（_tmp 檔案被 Excel 程式鎖住，請關閉 Excel 後重試存檔）")
    ...
    cl_page.close()
    ctx.close()
    time.sleep(4)  # 等 OS 釋放所有文件鎖

# ══════════════════════════════════════════════════════════════════════════════
# 瀏覽器已關閉：統一重命名資料夾（文件鎖已釋放）
# ══════════════════════════════════════════════════════════════════════════════
def _safe_move(src, dst, retries=15, delay=2):
    """重試移動文件，等 OS 釋放文件鎖（最多 retries * delay 秒）。"""
    for i in range(retries):
        try:
            ...
            if i < retries - 1:
                time.sleep(delay)
    print(f"  ⚠️  文件鎖未釋放，跳過：{os.path.basename(src)}")
    return False
...
for c in completed:
    ...
    name    = c["order"]["name"]
    old_dir = c["save_dir"]
    new_dir = c.get("save_dir_final", old_dir)
    ...
    for f in os.listdir(old_dir):
        ...
        if "_明細+清關" in f:
            dst = os.path.join(new_dir, f"{name}_{today}_{c['pos_no']}_{waybill}_明細+清關.pdf")
        elif "_運單" in f:
            dst = os.path.join(new_dir, f"{name}_{today}_{c['pos_no']}_{waybill}_運單.pdf")
        else:
            dst = os.path.join(new_dir, f)
        _safe_move(src, dst)
    shutil.rmtree(old_dir, ignore_errors=True)
    print(f"  📁 {os.path.basename(new_dir)}")
