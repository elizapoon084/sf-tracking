...
def download_pos_word(ctx, pos_order_no, save_dir, customer_name):
    ...
    try:
        ...
        # ── 收貨明細 / 清關：用 POS 搜尋框重試輔助函數 ──────────────────────
        def _ctrlf_refocus(page, order_no):
            """
            點 POS 頁面頂部搜尋框，輸入訂單號，
            等 3 秒讓 React filter + PDF blob 重新生成。
            注意：唔用瀏覽器 Ctrl+F，因為瀏覽器 Ctrl+F 只搜尋頁面文字，
            唔會觸發 React 重新 filter 訂單及重生成 blob PDF。
            """
            print(f"    [POS搜尋框] 搜尋 {order_no}，等待 React 重新渲染...")
            try:
                search_box = page.locator(
                    "input[placeholder*='搜尋單號'], "
                    "input[placeholder*='搜索单号'], "
                    "input[placeholder*='搜尋']"
                ).first
                search_box.wait_for(state="visible", timeout=5000)
                search_box.click()
                search_box.triple_click()
                search_box.type(order_no, delay=60)
                print(f"    [POS搜尋框] 已輸入 {order_no}，等 3 秒...")
                time.sleep(3)
            except Exception as se:
                print(f"    [WARN] 搜尋框定位失敗，改用 JS 直接設值: {se}")
                page.evaluate(f"""() => {{
                    const inputs = document.querySelectorAll('input');
                    for (const inp of inputs) {{
                        const ph = inp.placeholder || '';
                        if (ph.includes('搜尋') || ph.includes('搜索')) {{
                            const setter = Object.getOwnPropertyDescriptor(
                                window.HTMLInputElement.prototype, 'value').set;
                            setter.call(inp, '{order_no}');
        ...
        # ── 2. 收貨明細（下載 attribute 含「明細」，失敗則用 POS 搜尋框再試）──
        # DevTools 顯示 download="明細_ORD-XXXXX.pdf"，所以 keyword 係「明細」
        try:
            try:
            ...
            try:
                dl = _try_download_receipt()
            except Exception as first_err:
                print(f"  [WARN] 收貨明細第一次下載失敗: {first_err}")
                print("  [Retry] 用 POS 搜尋框重新 filter 訂單後再試...")
                _ctrlf_refocus(pos2, pos_order_no)
                try:
                    pos2.wait_for_function(f"""() => {{
                        for (const a of document.querySelectorAll('a[download]')) {{
                            const dl = a.getAttribute('download') || '';
                    ...
                    print("  [OK] 搜尋框 filter 後收貨明細 PDF 已就緒")
                except Exception:
                    print("  [WARN] 仍等不到 blob，繼續嘗試點擊")
                dl = _try_download_receipt()
            ...
            print(f"  ⚠️  收貨明細下載失敗（已重試）: {dl_err}")

        # ── 3. 清關PDF（失敗則用 POS 搜尋框再試）────────────────────────────
        try:
            try:
                pos2.wait_for_function(f"""() => {{
                    for (const a of document.querySelectorAll('a[download]')) {{
            ...
            try:
                dl = _try_download_customs()
            except Exception as first_err:
                print(f"  [WARN] 清關PDF第一次下載失敗: {first_err}")
                print("  [Retry] 用 POS 搜尋框重新 filter 訂單後再試...")
                _ctrlf_refocus(pos2, pos_order_no)
                try:
                    pos2.wait_for_function(f"""() => {{
                    ...
                    print("  [OK] 搜尋框 filter 後清關 PDF 已就緒")
                except Exception:
                    print("  [WARN] 仍等不到 blob，繼續嘗試點擊")
                dl = _try_download_customs()
            ...
            print(f"  ⚠️  清關PDF下載失敗（已重試）: {dl_err}")
